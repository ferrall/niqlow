...
//	if (I::t==39 && !CV(L) && !CV(g) ) println("**** ","b= ",b);
	return  exp( (1~CV(g)~I::t~sqr(I::t)~eps) * CV(beta) ) ; }
LSzB::Utility(){
	eps = probn(pstar[0]+pstar[1]*ranu(1,1));			// called in simulation
	return CV(m)*Earn() + (1-CV(m))*Home();
	}
...
LSzB::EUtility()    {
    decl pstar = 1-probn(zstar), sig =beta[3]; //this is the st. dev. assuming e is N(0,1)
    eps = sig/2;  //so e^{sig^2/2} is in Earn, to match E[exp(be)]
	return {  ( Home() | Earn()*probn((zstar/sig-sig)/pstar)) , (1-pstar)~pstar};
	}	
